...
if($search == null || $search == "" ){
      ...
      }else if($deadline != null && $search == null){
        $sqla="SELECT no, COUNT( * ) AS totaldata FROM $forward where tgldeadline = '$deadline'";
      }else if($deadline == null && $search == null && $date != null){
        $sqla="SELECT no, COUNT( * ) AS totaldata FROM $forward where tglmasuk = '$date'";
      }else{
...
?>
    ...
    if($search == null || $search == "" ){
      ...
      $sql="SELECT * FROM $forward where tgldeadline = '$deadline'";
    }else if($deadline == null && $search == null && $date != null){
      $sql="SELECT * FROM $forward where tglmasuk = '$date'";
    }else{
      $sql="SELECT *, bayar.nota as nota, bayar.no as no FROM  $forward left join pelanggan on pelanggan.kode = bayar.pelanggan where bayar.nota like '%$search%' or bayar.tglmasuk like '%$search%' or bayar.kasir like '%$search%' or pelanggan.nama like '%$search%'";
    }

    $result = mysqli_query($conn, $sql);
    $rpp    = 15;
    $reload = "$halaman"."?search=$search&pagination=true";
    ...
    if ($search != null || $search != "") {
        ...
        // if ($_SERVER["REQUEST_METHOD"] == "POST") {
				...
				$query1="SELECT *,bayar.nota as nota, bayar.no as no FROM  $forward inner join pelanggan on pelanggan.kode = bayar.pelanggan where bayar.nota like '%$search%' or bayar.tglmasuk like '%$search%' or bayar.kasir like '%$search%' or pelanggan.nama like '%$search%' order by bayar.no limit $rpp";
					...
					?>
                     <tbody>
                       ...
                       <?php }else if($fill['status'] == 'selesai'){ ?>
                         ...
                         <?php	if ($chmod >= 3 || $_SESSION['jabatan'] == 'admin') { ?>
                           <?php if($fill['status']=='proses'){?>
                             <button type="button" class="btn btn-success btn-xs no-print btn-flat" style="width:55px" onclick="window.location.href='component/setting/status_master?status=<?php echo 'selesai'.'&';?>no=<?php echo $fill['no'].'&'; ?>forward=<?php echo $forward.'&';?>forwardpage=<?php echo $forwardpage.'?search='.$search.'&'; ?>chmod=<?php echo $chmod.'&'; ?>'">Selesai</button>
                             <?php }else if($fill['status']=='selesai'){ ?>
                               <button type="button" class="btn btn-danger btn-xs no-print btn-flat" style="width:55px" onclick="window.location.href='component/setting/status_master?status=<?php echo 'lunas'.'&';?>no=<?php echo $fill['no'].'&'; ?>forward=<?php echo $forward.'&';?>forwardpage=<?php echo $forwardpage.'?search='.$search.'&'; ?>chmod=<?php echo $chmod; ?>'">Lunas</button>
                           <?php } ?>
			...
			?>
                      <tbody>
                        ...
                        <?php }else if($fill['status'] == 'selesai'){ ?>
                          ...
                          <?php	if ($chmod >= 3 || $_SESSION['jabatan'] == 'admin') { ?>
                            <?php if($fill['status']=='proses'){?>
                              <button type="button" class="btn btn-success btn-xs no-print btn-flat" style="width:55px" onclick="window.location.href='component/setting/status_master?status=<?php echo 'selesai'.'&';?>no=<?php echo $fill['no'].'&'; ?>forward=<?php echo $forward.'&';?>forwardpage=<?php echo $forwardpage.'?search='.$search.'&'; ?>page=<?php echo $page.'&'; ?>chmod=<?php echo $chmod.'&'; ?>'">Selesai</button>
                              <?php }else if($fill['status']=='selesai'){ ?>
                                <button type="button" class="btn btn-danger btn-xs no-print btn-flat" style="width:55px" onclick="window.location.href='component/setting/status_master?status=<?php echo 'lunas'.'&';?>no=<?php echo $fill['no'].'&'; ?>forward=<?php echo $forward.'&';?>forwardpage=<?php echo $forwardpage.'?search='.$search.'&'; ?>chmod=<?php echo $chmod; ?>'">Lunas</button>
                            <?php } ?>
                            <button type="button" class="btn btn-primary btn-xs no-print btn-flat" style="width:55px" onclick="window.open('print_one.php?nota=<?php  echo $fill['nota']; ?>')">Cetak</button>
